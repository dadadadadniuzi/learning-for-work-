...
- `event`：事件结构指针，类型是 [[linux网络编程/概念词条/epoll_event|struct epoll_event]] 指针。添加或修改时用于描述关注事件和用户数据；删除时在很多 Linux 实现中可传 `NULL`。
...
- 常见关注读事件写法是 `ev.events = EPOLLIN; ev.data.fd = fd;`，其中 `data` 字段类型是 [[linux网络编程/概念词条/epoll_data_t|epoll_data_t]]。
...
- `event.data.fd` 通常要保存目标 fd，否则 [[linux网络编程/函数笔记/IO多路复用/epoll_wait|epoll_wait]] 返回后不好判断是谁就绪。
